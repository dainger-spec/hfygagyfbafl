...
if (!TOKEN) {
  console.warn("[epb] TELEGRAM_BOT_TOKEN is not set");
}
...
const DEFAULT_PUBLIC_URL = "https://europe.bothost.tech";
...
export function verifyTelegramInitData(initData: string): { id: string; username?: string } | null {
...
export function allowEvidenceUpload(initData: string, bytes: number): { ok: true; userId: string } | { ok: false; error: string } {
  if (bytes > 50 * 1024 * 1024) return { ok: false, error: "file too large" };
  const user = verifyTelegramInitData(initData);
  if (!user) return { ok: false, error: "telegram auth required" };
  if (tooMany(`up:${user.id}`, 40, 60 * 60 * 1000)) return { ok: false, error: "too many uploads" };
  if (tooMany("up:global", 200, 60 * 60 * 1000)) return { ok: false, error: "server busy" };
  return { ok: true, userId: user.id };
}

function allowNewComplaint(userId: string, storyLen: number): string | null {
  if (storyLen > 4000) return "Слишком длинное описание.";
  if (tooMany(`app:${userId}`, 3, 6 * 60 * 60 * 1000)) return "Слишком много заявок. Подождите несколько часов.";
  if (tooMany("app:global", 40, 60 * 60 * 1000)) return "Сейчас слишком много заявок. Попробуйте позже.";
  return null;
}
...
export async function resolveHandleServer(input: string): Promise<{
  ...
  if (tooMany("resolve:global", 40, 60_000)) return null;
...
export async function sendApplicationToArbiters(
  ...
  if (app.evidence.length > 20) return { ok: false, error: "Слишком много файлов." };
  try {
...
async function submitSession(chatId: number, userId: string, session: ApplySession) {
  ...
  if (!accounts.length) {
    await tgSoft("sendMessage", { chat_id: chatId, text: "Добавьте хотя бы один аккаунт." });
...
async function handleApplyCallback(
  ...
  if (cb.data === "apply:next") {
    if (!session.accounts.length) {
      await tgSoft("sendMessage", { chat_id: chatId, text: "Сначала добавьте аккаунт." });
      return;
    }
    session.step = "victim";
    sessions.set(userId, session);
...
export async function handleUpdate(update: TgUpdate) {
  ...
  if (floodId && tooMany(`flood:${floodId}`, 45, 60_000)) {
    return;
  }
  ...
  if (msg.chat.type === "channel" && bindings.channelId && !isOfficialChannel(msg.chat.id)) {
    return;
  }
