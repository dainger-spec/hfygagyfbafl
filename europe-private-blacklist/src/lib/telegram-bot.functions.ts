...
export const submitToTelegram = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { sendApplicationToArbiters, verifyTelegramInitData } = await import("@/lib/telegram-bot.server");
    const authed = verifyTelegramInitData(data.initData || "");
    if (!authed) {
      return { ok: false as const, error: "Откройте Mini App из бота Telegram." };
    }
    if (data.victim.telegramId && data.victim.telegramId !== authed.id) {
      data.victim.telegramId = authed.id;
    }
    if (!data.victim.telegramId) data.victim.telegramId = authed.id;
